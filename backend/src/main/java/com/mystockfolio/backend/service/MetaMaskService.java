...
public class MetaMaskService {
    ...
    /**
     * Nonce 생성 (파라미터 없음 - 프론트엔드에서 지갑 주소를 보내지 않는 경우)
     */
    public MetaMaskDto.NonceResponse generateNonce() {
        // 랜덤 nonce 생성
        ...
        log.info("Nonce 생성 완료 (임시) - nonce: {}", nonce);
    ...
    public MetaMaskDto.AuthResponse verifyAndAuthenticate(String walletAddress, String signature, String nickname) {
        ...
        if (existingUser.isPresent()) {
            ...
            user = User.builder()
                    ...
                    .isOauth2Signup(false) // MetaMask는 즉시 완료
    ...
    /**
     * MetaMask 인증 (AuthRequest 사용)
     */
    @Transactional
    public com.mystockfolio.backend.dto.AuthDto.AuthResponse authenticateWithMetaMask(MetaMaskDto.AuthRequest request) {
        log.info("MetaMask 인증 시작 - walletAddress: {}", request.getWalletAddress());

        // 기존 verifyAndAuthenticate 메서드 호출
        MetaMaskDto.AuthResponse metaMaskResponse = verifyAndAuthenticate(
                request.getWalletAddress(),
                request.getSignature(),
                request.getNickname()
        );

        // AuthDto.AuthResponse로 변환하여 반환
        return com.mystockfolio.backend.dto.AuthDto.AuthResponse.builder()
                .userId(metaMaskResponse.getUserId())
                .email(metaMaskResponse.getEmail())
                .nickname(metaMaskResponse.getNickname())
                .accessToken(metaMaskResponse.getAccessToken())
                .build();
    }
