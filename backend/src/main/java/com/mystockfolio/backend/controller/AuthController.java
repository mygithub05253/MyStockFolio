...
public class AuthController {
    ...
    // MetaMask 논스 발급 API (GET /api/auth/metamask/nonce)
    @GetMapping("/metamask/nonce")
    public ResponseEntity<MetaMaskDto.NonceResponse> getMetaMaskNonce() {
        MetaMaskDto.NonceResponse nonce = metaMaskService.generateNonce();
        return ResponseEntity.ok(nonce);
    }

    // MetaMask 인증 API (POST /api/auth/metamask)
    @PostMapping("/metamask")
    public ResponseEntity<AuthDto.AuthResponse> authenticateWithMetaMask(@Valid @RequestBody MetaMaskDto.AuthRequest request) {
        AuthDto.AuthResponse authResponse = metaMaskService.authenticateWithMetaMask(request);
        return ResponseEntity.ok(authResponse);
    }
