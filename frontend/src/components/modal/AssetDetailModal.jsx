...
import useInput from '../../hooks/useInput'; // useInput import
...
const AssetDetailModal = ({ isOpen, onClose, asset, portfolioId }) => {
    const dispatch = useDispatch();
    const [isEditing, setIsEditing] = useState(false); // 수정 모드 상태
    const [error, setError] = useState('');
    
    // 수정 필드 상태 (useInput 활용)
    ...
    // 임시 시세 데이터 (P2-2 단계에서 FastAPI 연동 후 수정될 부분)
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            ...
            const updatedAsset = response.data; // 서버에서 반환된 수정된 자산 객체
