...
const AssetItem = ({ asset, portfolioId, onClick }) => {
    ...
    // 임시 데이터 (실제는 시장 데이터 서비스 연동 후 수정)
    // TODO: 이 부분은 P2-2 단계에서 FastAPI 연동 후 실제 데이터로 대체되어야 합니다.
    ...
    const returnRate = (initialValue > 0) ? (gainLoss / initialValue) * 100 : 0; // 0으로 나누는 것 방지
    ...
    const handleDelete = async (e) => {
        ...
        try {
            // [API 호출]: DELETE /api/portfolios/{portfolioId}/assets/{assetId}
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between text-sm mt-3">
                <div>
                    <p className="text-gray-600">수량: {asset.quantity.toFixed(4)}</p>
                    <p className="text-gray-600">평단: ₩{asset.avgBuyPrice.toFixed(2)}</p>
                </div>
